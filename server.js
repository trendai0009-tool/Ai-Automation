...
// ============================================================
// ─── N1N.AI API CALL FUNCTION (COST-OPTIMIZED - 90% CHEAPER) ────
// Models: GPT-4o-mini | DeepSeek-V3 | Llama-3.3-70b | GPT-4o
// Cost per generation: ₹0.01-0.09
// ================================================================
const COST_OPTIMIZED_MODELS = {
  // Office & Documents — GPT-4o-mini (cheap, good quality)
  'Microsoft Excel':       'gpt-4o-mini',
  ...
  'QuickBooks':            'gpt-4o-mini',
  'WordPad':               'gpt-4o-mini',

  // Accounting tasks — DeepSeek-V3 (best for numbers & GST)
  'Tally':                    'deepseek-v3',
  'Busy Accounting Software': 'deepseek-v3',

  // Simple plain text — Llama (cheapest & fastest)
  'Notepad': 'llama-3.3-70b',
};

const DEFAULT_COST_MODEL = 'gpt-4o-mini';

// Fallback chain (sasta to mehnga)
const COST_FALLBACK_CHAIN = [
  'gpt-4o-mini',   // Default cheap model
  'deepseek-v3',   // For accounting tasks
  'llama-3.3-70b', // Cheapest option
  'gpt-4o',        // Last resort — premium
];

// ─── N1N.AI CALL FUNCTION WITH AUTO FALLBACK ──────────────────
async function callSiliconFlow(systemPrompt, userContent, selectedPlatform) {
  // Platform ke liye optimized model select karo
  const primaryModel = COST_OPTIMIZED_MODELS[selectedPlatform] || DEFAULT_COST_MODEL;

  // Primary + fallbacks (no duplicates)
  ...
  for (let i = 0; i < modelsToTry.length; i++) {
    ...
    try {
      console.log(`🟢 n1n.ai Attempt ${i+1}: [${model}] → ${selectedPlatform}`);
      ...
      const response = await axios.post(
        'https://api.n1n.ai/v1/chat/completions',
        {
          ...
          messages:    messages,
          max_tokens:  4096,
          temperature: 0.3,  // Kam temperature = zyada accurate
          top_p:       0.9,
          stream:      false
        },
        {
      ...
      outputText = cleanExcelOutput(outputText, selectedPlatform);
      ...
      if (i === modelsToTry.length - 1) {
        throw new Error(`AI failed after ${modelsToTry.length} attempts. Last error: ${lastError}`);
      }
    }
  }
}

// ─── Helper function to clean Excel/Sheets output ─────────────
function cleanExcelOutput(output, platform) {
  if (platform !== 'Microsoft Excel' && platform !== 'Google Sheets') {
    return output;
  }

  // Remove duplicate columns and garbage lines
  const lines = output.split('\n');
  const cleanedLines = [];

  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    // Skip lines with obvious garbage (too many columns with duplicate headers)
    if (line.includes('ManagerID') && line.includes('ManagerName') && line.split('\t').length > 20) {
      continue;
    }
    if (line.match(/[A-Z0-9]{5,}-[A-Z0-9]{2,}-[A-Z0-9]{2,}/) && line.length < 30) {
      // Skip random codes like F3-GJ-H3-I3
      continue;
    }
